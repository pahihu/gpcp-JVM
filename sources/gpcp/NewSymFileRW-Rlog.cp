...
(*       THIS VERSION WITH LOGGING OF SYMFILE READER                    *)
(*                                                                      *)
(* ==================================================================== *)
...
(* ============================================================ *)
  ...
  PROCEDURE LgLn(IN m : ARRAY OF CHAR); 
  BEGIN
    Console.WriteString(logPrefix); 
    Console.WriteString(m); 
    Console.WriteLn; 
  END LgLn;

  PROCEDURE LgNL(); BEGIN Console.WriteString(logPrefix) END LgNL;

  PROCEDURE LgS(IN m : ARRAY OF CHAR);
  BEGIN
    Console.WriteString(m);
    Console.Write(" ");
  END LgS;

  PROCEDURE LgI(i : INTEGER);
  BEGIN 
    Console.WriteInt(i, 1); Console.Write(" ");
  END LgI;

  PROCEDURE LgEnd(); BEGIN Console.WriteLn END LgEnd;

  PROCEDURE LgTyp(t : D.Type);
  BEGIN
    IF t = NIL THEN Console.WriteString("nilTp "); RETURN; END;
    Console.WriteString(Ty.kindStr(t));
    Console.Write(' ');
    Console.Write('#');
  ...
  PROCEDURE LgTid(t : D.Idnt);
  BEGIN
    IF t.type = NIL THEN Console.WriteString("<type=NIL> ");
    ELSE LgTyp(t.type);
    END;
  END LgTid;    
...
LgEnd;
LgLn(" =======================================");
LgLn(message^ + ", Key: " + Lt.intToCharOpen(f.impS.modKey)^);
LgLn(BOX("Found " + BF.getFullPathName(f.file)^ + " ?"));
  FOR index := 0 TO f.sArray.tide - 1 DO
LgLn("  imports " + NameAndKey(f.sArray.a[index])^);
  END;
LgLn(" =======================================");
LgEnd;
      f.SymFile(filNm);
...
LgTid(parD);
    ...
    f.ReadPast(endFm);
LgS("}");
    IF rslt.retType # NIL THEN LgS(":"); LgTyp(rslt.retType) END; 
LgEnd;
    RETURN rslt;
  END getFormalType;
...
(* LOGGING *
LgLn( "CONST " + f.strAtt^ + " of type " + newC.type.name()^ );
 * *)
   RETURN newC;
  END constant;
...
(* ============================================ *)
  ...
  BEGIN
    ...
    newV.dfScp := f.impS;
(* LOGGING *)
LgLn( "VAR " + f.strAtt^ + " : " + newV.type.name()^ );
    RETURN newV;
  END variable;
...
LgS("<==");
      ...
      | arrSy : LgS("ARRAY");     
                rslt := f.arrayType();      
LgS("OF"); LgTyp(rslt(Ty.Array).elemTp); LgEnd;
      | vecSy : LgS("VECTOR");    
                rslt := f.vectorType();     
LgS("OF"); LgTyp(rslt(Ty.Vector).elemTp); LgEnd;
      | recSy : LgS("RECORD");    
                rslt := f.recordType(p); LgEnd; 
LgNL; LgS("  xtnds");  LgTyp(rslt(Ty.Record).baseTp); LgEnd;
      | pTpSy : LgS("PROCTYPE"); LgEnd; 
                rslt := f.procedureType();
      | evtSy : LgS("EVENT"); LgEnd;     
                rslt := f.eventType();
      | eTpSy : LgS("ENUM"); LgEnd;      
                rslt := f.enumType();
      | ptrSy : LgS("POINTER");   
                rslt := f.pointerType(p);   
LgS("TO"); LgTyp(rslt(Ty.Pointer).boundTp); LgEnd;
      ELSE                        
                rslt := Ty.newNamTp();      
LgTyp(rslt); LgEnd;
      END;
  ...
  BEGIN
(* LOGGING *)
LgLn("Starting TypeList Processing");
...
(* LOGGING *)  
LgNL(); LgI(typOrd); LgS("-> prevTp"); LgTyp(prevTp);
      f.ReadPast(tDefS);
     (*
...
LgS("FROM " + impScp.namStr + ", " + f.strAtt^ );
...
(* LOGGING *)
    ...
    FOR linkIx := 0 TO f.tArray.tide - 1 DO
      ...
      *)
(* LOGGING *)
LgNL; LgS("TypeFix"); LgTyp(tpDesc); LgEnd;
...
LgNL; LgS("baseT check"); LgTyp(tpDesc);
...
LgS("baseT"); LgTyp(prevTp); LgEnd;
    ...
    *)
    ...
    END;
LgLn("Ending TypeList Processing");
  END TypeList;
...
(* ============================================ *)
  ...
  BEGIN
    ...
    LOOP
      ...
      END;
    END;
LgLn("Ending definitions for " + nm);
LgLn(" =======================================");
...
(* ============================================================ *)
  ...
  BEGIN
    ...
    ELSIF id.type.kind = Ty.tmpTp THEN
      ...
      IF id.type.serial # oldT.serial THEN
LgNL; LgTyp(oldT); LgS("<=="); LgTyp(id.type); LgEnd;
      END;
    ELSE
